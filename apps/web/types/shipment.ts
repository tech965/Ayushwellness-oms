...
export interface ShipmentQueueRow {
  ...
  awb: string | null
  courier_name: string | null
  // The real, numeric Shiprocket order id for this row's shipment --
  // `null` exactly when `shipment_id` is `null`, or when the OMS has no
  // reliably-stored Shiprocket order id for it. Shiprocket has no
  // supported deep-link URL for a specific order, so "Process
  // Shipment"/"Ship Order" open Shiprocket's plain "Ready to Ship" page
